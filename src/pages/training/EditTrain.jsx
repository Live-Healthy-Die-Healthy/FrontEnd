...
const EditTrain = () => {
  ...
  useEffect(() => {
    const fetchExerciseLog = async () => {
      try {
        const response = await axios.post('http://localhost:4000/getTrainingLog', {
        ...
        if (response.status === 200) {
          ...
          if (data.exerciseType === "AerobicExercise") {
            ...
            setSets(data.weight.map((weight, index) => ({
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await axios.put('http://localhost:4000/exerciseLog', exerciseData, {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      });
